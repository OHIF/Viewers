...
// const NON_IMAGE_MODALITIES = ['ECG', 'PR'];
...
//   measurements: '@ohif/extension-cornerstone.panelModule.panelMeasurementWithTools',
...
//     id,
//     routeName: 'viewer',
//     displayName: i18n.t('Modes:General Viewer'),
//     /**
//      * Lifecycle hooks
//      */
...
//       // Add the segmentation buttons to the toolbar
...
function modeFactory({ modeConfiguration }) {
  ...
  return {
    ...
    onModeEnter: function ({ servicesManager, extensionManager, commandsManager }) {
      ...
      const labelConfig = modeConfiguration?.labelConfig || null;

      measurementService.clearMeasurements();
      // performCustomizations(customizationService);

      initToolGroups(extensionManager, toolGroupService, commandsManager, labelConfig);
    ...
    isValidMode: function ({ modalities }) {
      ...
      return {
        valid: !!modalitiesList.find(modality => !NON_IMAGE_MODALITIES.includes(modality)),
        description:
          'The mode does not support studies that ONLY include the following modalities: SM, ECG, SEG, RTSTRUCT',
      };
    },
