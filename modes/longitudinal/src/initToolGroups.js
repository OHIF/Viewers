  ...
  const tools = {
    ...
    passive: [
      { toolName: toolNames.Length },
      {
        toolName: toolNames.ArrowAnnotate,
        configuration: {
          getTextCallback: (cb, evt) =>
            modeLabelConfig ? cb(' ') : commandsManager.runCommand('arrowTextCallback', { callback: cb, eventDetails: evt }),
          changeTextCallback: (data, evt, cb) =>
            modeLabelConfig === undefined && commandsManager.runCommand('arrowTextCallback', { callback: cb, data, eventDetails: evt }),
        },
      },
      { toolName: toolNames.Bidirectional },
      { toolName: toolNames.DragProbe },
...
function initSRToolGroup(extensionManager, toolGroupService, commandsManager) {
  const SRTools = extensionManager.getModuleEntry('@ohif/extension-cornerstone-dicom-sr.utilityModule.tools');
  if (!SRTools) return;

  const CSTools = extensionManager.getModuleEntry('@ohif/extension-cornerstone.utilityModule.tools');
  const { toolNames: SRToolNames } = SRTools.exports;
  const { toolNames, Enums } = CSTools.exports;
...
function initMPRToolGroup(extensionManager, toolGroupService, commandsManager, modeLabelConfig) {
  const utilityModule = extensionManager.getModuleEntry('@ohif/extension-cornerstone.utilityModule.tools');
  const { cornerstoneViewportService } = extensionManager._servicesManager.services;
  const { toolNames, Enums } = utilityModule.exports;
