...
test('should toggle all segments visibility - on/off', async ({
  ...
  await expect(
    svgPathLocator,
    'Expected no SVG paths after toggling all visibility off'
  ).toHaveCount(0);
  ...
  await expect(
    svgPathLocator,
    'Expected SVG path count to match initial after toggling all back on'
  ).toHaveCount(2);
});
...
test('should restore svg paths when segment visibility is toggled on/off', async ({
  ...
  await segment0.toggleVisibility();
  await expect(
    (await viewportPageObject.getById('default')).svg('path'),
    'No segment to be displayed'
  ).toHaveCount(0);
