...
if (Meteor.isClient) {
    ...
    const emptyRoute = {
        onRun: function() {
            console.warn('Empty Route: onRun');
            // Retrieve the query from the URL the user has entered
            const standaloneData = window.localStorage.getItem('ohif-standalone-data');
            if (!standaloneData) {
                throw new Error('No launch data was provided.');
            }
            
            const query = standaloneData.query;
            const id = standaloneData.id;
    ...
    Router.route('', emptyRoute);
    Router.route('index.html', emptyRoute);
