...
window.config = {
  ...
  // OFF by default. Set to true (or pass ?useNextViewports=true in the URL) to
  // drive viewports through cornerstone's native GenericViewport ("next") API
  // instead of the legacy Stack/Volume viewport classes.
  useNextViewports: false,
  // Render backend selection: 'cpu' | 'webgl' | 'auto' | a backend id registered
  // via cornerstone's registerRenderBackend (e.g. a webgpu backend), or a map
  // with per-viewport-type overrides, e.g. { default: 'webgl', orthographic: 'cpu' }.
  // The matching URL params take precedence per-session:
  // ?viewportRendering=cpu and ?orthographic.viewportRendering=cpu.
