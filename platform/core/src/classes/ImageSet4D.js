 ...
 */
class ImageSet {
  constructor(images, sortTag4DForInstances) {
    if (Array.isArray(images) !== true) {
      throw new OHIFError('ImageSet expects an array of images');
    }

    const imageStacks = [];
    const { tagName, tagValues } = sortTag4DForInstances;
    ...
    images.forEach(image => {
      const tagValue = image.getData().metadata[tagName];
      const index = tagValues.indexOf(tagValue);

      imageStacks[index].push(image);
    });

    imageStacks.forEach(imageStack =>
  ...
  sortByImagePositionPatient(images) {
