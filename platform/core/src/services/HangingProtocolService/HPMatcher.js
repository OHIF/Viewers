...
const match = (metadataInstance, rules, customAttributeRetrievalCallbacks, extraMatchData) => {
  ...
  rules.forEach(rule => {
    const { attribute, source } = rule;
    const metadataSource = source ? extraMatchData[source] : metadataInstance;
    ...
    if (customAttributeRetrievalCallbacks.hasOwnProperty(attribute)) {
      readValues[attribute] = customAttributeRetrievalCallbacks[attribute].callback(metadataSource, extraMatchData);
    } else {
      readValues[attribute] = metadataSource[attribute] ??
    ...
    if (!errorMessages) {
      ...
      // console.log("passed readValues", attribute, readValues);
      ...
      if (rule.required) {
        requiredFailed = true;
        // console.log("Failed required readValues", attribute, readValues, rule, errorMessages)
      } else {
        // console.log("Failed not required readValues", readValues);
      }
