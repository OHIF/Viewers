...
- `experimentalStudyBrowserSort`: boolean, if set to true, you will get the experimential StudyBrowserSort compontent in the UI, which displays a list of sort functions that the displaySets can be sorted by, the sort reflects in all part of the app including the thumbnail/study panel. These sort functions are defined in the customizationModule and can be expanded by users.
