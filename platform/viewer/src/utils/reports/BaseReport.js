...
export default class BaseReport {
  ...
  init() {
    this.doc = new jsPDF('portrait', 'pt', [
      this.options.width,
      this.options.height,
    ]);
  ...
  printHeader() {
    ...
    doc.roundedRect(marginLeft + 0.5, y + 0.5, 8, 8, 0.5, 0.5, 'DF');  // 'D');
    doc.roundedRect(marginLeft + 11, y + 0.5, 8, 8, 0.5, 0.5, 'DF');  // 'D');
    doc.roundedRect(marginLeft + 0.5, y + 11, 8, 8, 0.5, 0.5, 'DF');  // 'D');
    doc.roundedRect(marginLeft + 11, y + 11, 8, 8, 0.5, 0.5, 'DF');  // 'D');
  ...
  printPageNumber() {
    ...
    doc.text(
      text,
      width - marginRight - size.w,
      height - marginBottom + (size.h / 2)
    );
  }
