import { AllInOneMenu, Button, InputFilterText } from '@ohif/ui';
import React, { ReactElement, useState, useEffect, useCallback } from 'react';
import { Icon } from '@ohif/ui';
import { ButtonEnums } from '@ohif/ui';
import {
  ViewportPreset,
  VolumeRenderingProps,
  VolumePresetsProps,
} from '../../types/ViewportPresets';

const formatLabel = (label: string, maxChars: number) => {
  return label.length > maxChars ? `${label.slice(0, maxChars)}...` : label;
};

function VolumePresets({ presets, viewportId, commandsManager, onClose }: VolumePresetsProps) {
