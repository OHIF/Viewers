...
/**
 * Modified Range Slider based on ReactSlider
 * Credit goes to https://github.com/zillow/react-slider
 */
...
class ReactSlider extends React.Component {
  ...
  static propTypes = {
    ...
    movableTrackIndex: PropTypes.number,
  };
  ...
  constructor(props) {
    ...
    this.throttledMoveTrack = throttle(this.moveTrack, 16, {
      leading: true,
      trailing: true,
    });
  }
  ...
  onTrackMouseUp = () => {
    this.onEnd({
      mousemove: this.onTrackMouseMove,
      mouseup: this.onTrackMouseUp,
    });
  };
  ...
  onTrackMouseMove = e => {
    // this.throttledOnTrackMouseMove(e);
    // Prevent controlled updates from happening while mouse is moving
    this.setState({ pending: true });

    const position = this.getMousePosition(e);
    const diffPosition = this.getDiffPosition(
      position[0],
      'trackStartPosition'
    );
    // const newValue = this.getValueFromPosition(diffPosition, 'trackStartValue');
    // this.moveTrack(diffPosition);
    this.throttledMoveTrack(diffPosition);
  };
  ...
  getValueFromPosition(position, startValueKey = 'startValue') {
    const diffValue =
      (position / (this.state.sliderLength - this.state.thumbSize)) *
      (this.props.max - this.props.min);
    return trimAlignValue(this.state[startValueKey] + diffValue, this.props);
  }

  getDiffPosition(position, startPositionKey = 'startPosition') {
    let diffPosition = position - this.state[startPositionKey];
  ...
  createMovableTrackOnMouseDown = e => {
    // do nothing if disabled or right click
    if (this.props.disabled || e.button === 2) {
      return;
    }
    ...
    const position = this.getMousePosition(e);
    this.startMovingTrack(position[0]);
    addHandlers({
      mousemove: this.onTrackMouseMove,
      mouseup: this.onTrackMouseUp,
    });
  ...
  startMovingTrack(position) {
    this.setState(prevState => ({
      trackStartValue:
        prevState.value[0] +
        Math.floor((prevState.value[1] - prevState.value[0]) / 2),
      trackStartPosition:
        position !== undefined ? position : prevState.trackStartPosition,
    }));
  }
  ...
  /*
  moveTrack(trackValue) {
    const { value, trackStartValue } = this.state;
    const { min, max } = this.props;

    if (trackValue === trackStartValue) {
      return;
    }

    const shift = trackValue - trackStartValue;
    const newValue = value.map(v => v + shift);

    const range = value[1] - value[0];

    if (newValue[0] < min) {
      newValue[0] = min;
      newValue[1] = min + range;
    }

    if (newValue[1] > max) {
      newValue[1] = max;
      newValue[0] = max - range;
    }

    this.hasMoved = true;

    this.setState({ value: newValue }, this.fireChangeEvent.bind(this, 'onChange'));
  }
  */
  ...
  renderTrack = (i, offsetFrom, offsetTo) => {
    ...
    const { movableTrackIndex } = this.props;
    if (movableTrackIndex !== undefined && movableTrackIndex === i) {
      props = {
        ...props,
        'onMouseDown': this.createMovableTrackOnMouseDown,
        // 'onTouchStart': this.createOnTouchStart(i),
      };
    }
    const state = {
