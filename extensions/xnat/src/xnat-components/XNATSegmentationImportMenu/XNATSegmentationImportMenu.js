...
export default class XNATSegmentationImportMenu extends React.Component {
    ...
    _hasExistingMaskData(firstImageId) {
        // TODO: Update this to use modern segmentation service
        // For now, return false to avoid using legacy cornerstone-tools
        return false;
    }
    ...
    render() {
        ...
        const sessionSelector = ( <
            ...
            <
            h5 > Session < /h5> <
            select onChange = { this.onSessionSelectedChange }
            value = { sessionSelected } > {
                Object.keys(sessionRoiCollections).map(key => {
                    ...
                    return ( <
                        option key = { key }
                        value = { key }
                        disabled = { session.importList.length === 0 } > { `${session.experimentLabel}` } <
                        /option>
                    );
                })
            } <
        ...
        importList.forEach(roiCollection => {
            if (!referencedSeriesNumberList.includes(
        ...
        if (importListReady) {
            if (importing) {
                ...
                importBody = ( <
                    ...
                    th width = "30%" >
                    Referenced Scan# <
                    select onChange = { this.onSelectedScanChange }
                    value = { scanSelected }
                    style = {
                        { display: 'block', width: '100%' }
                    } > {
                        referencedSeriesNumberList.map(seriesNumber => ( <
                            option key = { seriesNumber }
                    ...
                    tbody > {
                        ...
                        .map(roiCollection => ( <
                            ...
                            <
                            input className = "checkboxInCell"
                            type = "radio"
                            onChange = {
                                evt =>
                            ...
                            /> < /
                            td > <
                            td className = "left-aligned-cell" > { roiCollection.name } <
                            /td> <
                            td >
                            <
                            ...
                            td > <
                            td className = "centered-cell" > { `${roiCollection.referencedSeriesNumber}` } <
                            /td> < /
                            tr >
                        ))
                    } <
