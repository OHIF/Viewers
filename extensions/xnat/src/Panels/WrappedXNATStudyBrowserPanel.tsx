...
function WrappedXNATStudyBrowserPanel({ extensionManager, servicesManager, commandsManager }) {
  ...
  useEffect(() => {
    ...
    // Request display sets explicitly to ensure they're created
    const requestDisplaySets = async () => {
      if (StudyInstanceUIDs && StudyInstanceUIDs.length > 0) {
        console.log('XNAT: Explicitly requesting display sets creation for XNAT study browser');
        
        // Store UIDs in sessionStorage for retrieval
        try {
          sessionStorage.setItem('xnat_studyInstanceUID', StudyInstanceUIDs[0]);
        } catch (e) {
          console.warn('XNAT: Failed to store study UID in session', e);
        }
        
        // Directly call display set creation if available
        try {
          const dataSourceWithDisplaySets = extensionManager.getActiveDataSource()[0];
          if (dataSourceWithDisplaySets && 
              typeof dataSourceWithDisplaySets.retrieveSeriesMetadata === 'function') {
            await dataSourceWithDisplaySets.retrieveSeriesMetadata({
              StudyInstanceUID: StudyInstanceUIDs[0],
              madeInClient: true
            });
            console.log('XNAT: Successfully requested series metadata');
          } else {
            console.warn('XNAT: Could not access retrieveSeriesMetadata method');
          }
        } catch (error) {
          console.error('XNAT: Error requesting display sets:', error);
        }
      }
    };
    
    // Call immediately to ensure display sets are created
    requestDisplaySets().then(() => {
      console.log('XNAT: Display set request complete, mapping display sets');
      // Wait briefly for display sets to be processed
      setTimeout(() => {
        console.log('XNAT: Current display sets:', displaySetService.activeDisplaySets);
        
        // Update studies list only after request completes
        const currentDisplaySets = displaySetService.activeDisplaySets;
        if (currentDisplaySets && currentDisplaySets.length > 0) {
          // Map display sets to study browser format
          ...
          console.warn('XNAT: No display sets available after explicit request');
        }
    ...
    <div style={panelStyles.container}>
      {isLoading ? (
        <div className="loading-indicator">
          <div className="loading-text">Loading studies...</div>
        </div>
      ) : (
        <XNATStudyBrowser
          studies={studies}
