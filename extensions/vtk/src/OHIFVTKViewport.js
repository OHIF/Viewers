...
class OHIFVTKViewport extends Component {
  ...
  static getCornerstoneStack(imageIds, frameIndex) {
    // Clone the stack here so we don't mutate it
    const stack = Object.assign(
      {},
      {
        imageIds,
        currentImageIdIndex: frameIndex !== undefined ? frameIndex : 0,
        // frameIndex could be found with sopCommonModule, cornerstone
        // the sopCommonModule for any imageId that has a matching sopInstanceUID
      }
    );

    // Use to set FrameIndex one layer up?
    // if (sopInstanceUid) {
  ...
  getViewportData = (
    ...
    const stack = OHIFVTKViewport.getCornerstoneStack(imageIds, frameIndex);
    ...
    switch (sopClassUid) {
      ...
      default:
        ...
        return loadImageData(imageDataObject).then(() => {
          console.log('Done loading imageData...');
          return {
            data: imageDataObject.vtkImageData,
          };
        });
  ...
  async setStateFromProps() {
    ...
    console.log('setStateFromProps', sopClassUid);
  ...
  componentDidUpdate(prevProps) {
    const { displaySetInstanceUid, sopInstanceUid, frameIndex } = this.props;
    const {
      prevDisplaySetInstanceUid,
      prevSopInstanceUid,
      prevFrameIndex,
    } = prevProps;
    ...
    if (
      ...
      sopInstanceUid !== prevSopInstanceUid ||
  ...
  render() {
    const { children, viewportIndex } = this.props;
