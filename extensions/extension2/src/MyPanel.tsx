...
//   const [studyUID, setStudyUID] = useState<string | undefined>(undefined);
//   const [segmentationDone, setSegmentationDone] = useState(false);

//   const routerService = servicesManager.services?.routerService;
...
//     await studiesManager?.reloadStudyMetadata(studyUID);
    
//     const allSets = studiesManager?.getDisplaySets(studyUID) || [];
...
//   //     return alert('Please select a scope and at least one organ.');
//   //   }
//   //   setSegmentationDone(false); // Optional, at start of handleSegment
//   //   setLoading(true);
//   //   showMessage('🔍 Preparing segmentation...');

//   //   try {
//   //     if (!studyUID) throw new Error('Missing StudyInstanceUID from URL.');
...
//   //     showMessage('📡 Sending Bio-Grid server...');
...
//   //     setSegmentationDone(true);  // After success

//   //     setTimeout(displayOverlays, 1000);
//   //   } catch (err: any) {
...
//     // try {
//     //   if (!studyUID) {
//     //     throw new Error('Missing StudyInstanceUID from URL.');
//     //   }
...
//   //   // 🧠 Extract active SeriesInstanceUID from the current viewport
//   //   const { activeViewportId } = servicesManager.viewportGridService.getState();
//   //   const viewportData = servicesManager.viewportGridService.getViewportData(activeViewportId);
//   //   const displaySetInstanceUID = viewportData?.displaySetInstanceUID;

//   //   const displaySet = servicesManager.displaySetService.getDisplaySetByUID(displaySetInstanceUID);
//   //   const seriesUID = displaySet?.SeriesInstanceUID;

//   //   if (!seriesUID) {
//   //     throw new Error('Missing SeriesInstanceUID from viewer.');
//   //   }
...
//     // ✅ Dynamically extract StudyInstanceUID from active display set
...
//     if (!studyUID) {
//       showMessage('❌ StudyInstanceUID not found', 'error');
//       return;
//     }
...
//     const uid = params.get('StudyInstanceUIDs')?.split(',')[0];
//     setStudyUID(uid);
//   }, []);
...
//       {/* Navigation to Segmentation Mode */}
//       {/* {!loading && studyUID && (
//         <div className="mt-6">
//           <button
//             onClick={() =>
//               (window.location.href = `/viewer?StudyInstanceUIDs=${studyUID}`)
//             }
...
//               const patientID = displaySetService.getStudyMetadata(studyUID)?.PatientID;
//               const encodedPatientID = encodeURIComponent(patientID || '');
//               window.location.href = `/segmentation?StudyInstanceUIDs=${studyUID}&PatientID=${encodedPatientID}`;
//             }}
//             className="block w-full border border-blue-500 text-blue-400 py-2 rounded hover:bg-blue-800 text-sm"
//           >
//             🧬 Open in Segmentation Mode
