const convertSRToMeasurements = (Part10SRArrayBuffer) => {
...
const imagingMeasurementsToMeasurementData = (dataset, displaySets) => {
    ...
    OHIF.log.info(lengthStates);

    const getInstance = (lengthState) => {
        let instance;
    ...
    lengthStates.forEach(lengthState => {
        const instance = getInstance(lengthState);
        const imageId = OHIF.viewerbase.getImageId(instance, lengthState.ReferencedFrameNumber);
