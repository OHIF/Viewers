...
OHIF.viewer.toggleLesionTrackerTools = () => {
...
OHIF.viewer.toggleLesionTrackerToolsButtons = (isEnabled) => {
    const toolManager = Viewerbase.toolManager;
    const toolStates = previousStates || toolManager.getToolDefaultStates();

    if (isEnabled) {
        toolStates.disabledToolButtons = [];
        OHIF.viewer.toggleLesionTrackerToolsHotKeys(true);
    } else {
        ...
        OHIF.viewer.toggleLesionTrackerToolsHotKeys(false);
    }
...
OHIF.viewer.toggleLesionTrackerToolsHotKeys = (isEnabled) => {
