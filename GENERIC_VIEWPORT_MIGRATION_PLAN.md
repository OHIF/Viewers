...
- **M0 — Scaffolding & flag (no behavior change when off).** Add `useNextViewports`; extract the
...
- **M1 — Planar stack, read-only.** `'stack'`→`PLANAR_NEXT`; `nextBackend._setStackViewport` uses
...
- **M2 — Planar volume + MPR.** `'volume'`→`PLANAR_NEXT`, data layer passes `volumeId`; replace the
...
- **M3 — Presentation, VOI/colormap, synchronizers.** Split read/write into geometry (`setViewState`
...
- **M4 — Segmentation (labelmap).** Move rep selection off `isVolume*` to capability/content; use
  `addLabelmapRepresentationToViewportMap` + `useSliceRendering`; eliminate
  `convertStackToVolumeViewport`. **CS deps:** CS-13, CS-3, CS-6.
- **M5 — Crosshairs / reference lines / MPR tools.** Almost entirely gated on upstream CS-8/CS-10
...
- **M6 — 3D / video / WSI / ECG.** `VOLUME_3D_NEXT` (CS-14 VR API; 3D VR commands stay legacy under
  the flag until the API exists), `VIDEO_NEXT`/`WHOLE_SLIDE_NEXT` (already near-native; pass UID),
  `ECG_NEXT` via `setDisplaySets`.
- **M7 — Default-on & cleanup.** Flip default to `true`; after one release with both CI lanes green,
